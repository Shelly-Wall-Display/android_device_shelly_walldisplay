...
#define VOL_1000 1000000
	...
	if (On) {
		...
		if ((currSensorName && (0 == strcmp(currSensorName, "imx135mipiraw"))) ||
			...
			&& (0 == strcmp(SENSOR_DRVNAME_GC8034_MIPI_RAW, currSensorName))&&(pinSetIdx == 1)) {
			PK_ERR("[PowerON]pinSetIdx:%d, currSensorName: SENSOR_DRVNAME_GC8034_MIPI_RAW %s\n", pinSetIdx, currSensorName);
		/* First Power Pin low and Reset Pin Low */
		...
		if (GPIO_CAMERA_INVALID != pinSet[1-pinSetIdx][IDX_PS_CMRST])  //set sub camera reset to low when  main camera power on
			...
			/* VCAM_D */
			if (TRUE != _hwPowerOn(VCAMD, VOL_1200)) {//1250
				PK_DBG
					("[CAMERA SENSOR] Fail to enable analog power (VCAM_A),power id = %d\n",
						VCAMA);
				goto _kdCISModulePowerOn_exit_;
			}
			//mt6335_upmu_set_rg_vcamd_cal(13);
			mdelay(1);
			...
			&& (0 == strcmp(SENSOR_DRVNAME_GC8034MAIN_MIPI_RAW, currSensorName))&&(pinSetIdx==0)) {
			/* First Power Pin low and Reset Pin Low */
			PK_ERR("[PowerON]pinSetIdx:%d, currSensorName: SENSOR_DRVNAME_GC8034MAIN_MIPI_RAW %s\n", pinSetIdx, currSensorName);
			...
			//mt6335_upmu_set_rg_vgp3_cal(13);
			mdelay(1);

			/* VCAM_A */
			/*if (TRUE != _hwPowerOn(VGP1, VOL_2800)) {
				PK_DBG
					("[CAMERA SENSOR] Fail to enable analog power (VCAM_A),power id = %d\n",
						VCAMA);
				goto _kdCISModulePowerOn_exit_;
			...
			mdelay(5);
			   ...
			   && (0 == strcmp(SENSOR_DRVNAME_GC6153_SERIAL_YUV, currSensorName))&&(2==pinSetIdx)) {
			PK_ERR("[PowerON]pinSetIdx:%d, currSensorName: SENSOR_DRVNAME_GC6153_SERIAL_YUV %s\n", pinSetIdx, currSensorName);

			mtkcam_gpio_set(pinSetIdx, CAM2PDN,1);
			mdelay(3);
			if (TRUE != _hwPowerOn(VCAMA, VOL_2800)) {
				PK_DBG
				    ("[CAMERA SENSOR] Fail to enable analog power (VCAM_A),power id = %d\n",
			...
			ISP_MCLK1_EN(1);//8029 is mclk2
			mdelay(1);
			mtkcam_gpio_set(pinSetIdx, CAM2PDN,0);
			mdelay(1);
			mtkcam_gpio_set(pinSetIdx, CAM2PDN,1);
			mdelay(1);
			mtkcam_gpio_set(pinSetIdx, CAM2PDN,0);
		...
		/*
		if (!((pinSetIdx == 0 && (0 == strcmp(SENSOR_DRVNAME_GC8034MAIN_MIPI_RAW, currSensorName)))
			||(pinSetIdx == 1 && (0 == strcmp(SENSOR_DRVNAME_GC8034_MIPI_RAW, currSensorName))))){
		if (pinSetIdx == 0)
			ISP_MCLK1_EN(0);
		else if (pinSetIdx == 1)
			ISP_MCLK2_EN(0);
		...
		(0 == strcmp(SENSOR_DRVNAME_GC8034_MIPI_RAW, currSensorName))&&(pinSetIdx == 1)) {
			/* Set Power Pin low and Reset Pin Low */
		PK_ERR("[PowerOFF]pinSetIdx:%d, currSensorName: SENSOR_DRVNAME_GC8034_MIPI_RAW## %s\n", pinSetIdx, currSensorName);
			...
			/* VCAM_D */
			//mt6335_upmu_set_rg_vcamd_cal(0);
			if (TRUE != _hwPowerDown(VCAMD)) {
				PK_DBG
					("[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO),power id = %d\n",
						VCAMD);
			...
			if (TRUE != _hwPowerDown(VCAMIO)) {
				...
				goto _kdCISModulePowerOn_exit_;
					...
					(0 == strcmp(SENSOR_DRVNAME_GC8034MAIN_MIPI_RAW, currSensorName))&&(pinSetIdx==0)) {
			PK_ERR("[PowerOFF]pinSetIdx:%d, currSensorName: SENSOR_DRVNAME_GC8034MAIN_MIPI_RAW## %s\n", pinSetIdx, currSensorName);
			...
			if (TRUE != _hwPowerDown(VCAMIO)) {
			   ...
			   (0 == strcmp(SENSOR_DRVNAME_GC6153_SERIAL_YUV, currSensorName))&&(2==pinSetIdx)) {
			PK_ERR("[PowerOFF]pinSetIdx:%d, currSensorName: SENSOR_DRVNAME_GC6153_SERIAL_YUV## %s\n", pinSetIdx, currSensorName);
			mtkcam_gpio_set(pinSetIdx, CAM2PDN,0);
			mdelay(1);
			mtkcam_gpio_set(pinSetIdx, CAM2PDN,1);
			mdelay(1);
			ISP_MCLK1_EN(0);//close when ISP_MCLK1_EN(0) h8029 clk2
			mdelay(1);
			if (TRUE != _hwPowerDown(VCAMA)) {
				PK_DBG
					("[CAMERA SENSOR] Fail to enable analog power (VCAM_A),power id = %d\n",
